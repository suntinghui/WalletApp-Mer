	...
	export default {
		...
		methods: {
			...
			checkInput: function(e) {
				//定义表单规则
				var rule = [
					{name:"oldPassword", checkType : "notnull", checkRule:"",  errorMsg:"请输入原密码"},
					{name:"newPassword", checkType : "notnull", checkRule:"",  errorMsg:"请输入新密码"},
					{name:"confirmPassword", checkType : "same", checkRule:this.info.newPwd,  errorMsg:"两次新密码输入不一致"},
				];
				//进行表单检查
			...
			updatePwdAction: function(e) {
				...
				uni.request({
				    ...
				    data: {
				        oldLoginPassword: _this.info.oldPwd,
						loginPassword: _this.info.newPwd
				    },
				    success: (res) => {
